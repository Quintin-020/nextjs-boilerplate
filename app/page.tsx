...
const highlightPython = (code: string) => {
  return (
    code
      ...
      // keywords
      .replace(
        /\b(print|def|return|if|else|elif|for|while|in|import|from|as|class|try|except|finally|with|lambda)\b/g,
            ...
            <section key={i} className="flex flex-col gap-3">
              ...
              <div className="relative">
                <button
                  ...
                  className="absolute right-3 top-3 z-10 inline-flex h-8 items-center justify-center rounded-full px-3 text-xs font-semibold bg-white/10 text-zinc-100 backdrop-blur border border-white/15 hover:bg-white/15 transition"
                >
                  {copiedIndex === i ? "Copied!" : "Copy"}
                </button>

                <pre className="overflow-x-auto rounded-xl border border-black/10 bg-[#0d1117] p-5 pr-20 text-sm leading-6 text-zinc-100 dark:border-white/10 font-mono">
                  <code
                    dangerouslySetInnerHTML={{
                      __html: highlightPython(b.code),
                    }}
                  />
                </pre>
              </div>
